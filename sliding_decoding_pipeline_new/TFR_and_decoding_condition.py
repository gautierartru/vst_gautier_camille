...
subjects_list    = config.preprocessed_subjects_list[:6]                # list of participants (sid)
...
# labels
y = np.concatenate(duration_conditions)
print(y.shape)
...
fig, ax = decoding.plot_decoding_scores(
    ...
    plot_title = f"Sensor space decoding on {epochs_type} epochs between Long and Short conditions",
    plotting_theme="ticks"
)

os.makedirs(report_dir, exist_ok=True)
plt.savefig(report_dir + f'beta_sliding_decoding_condition_{pick_type}_plot.png')
