...
for ic, condition_pair in enumerate(config.condition_pairs):
    report_out  = decoding_report_dir + f'popauc{"_".join(condition_pair)}.html'
    report      = Report(report_out, verbose=False)    

    for isub, subject in enumerate(subjects_list):

        auc_in  = decoding_dir + subject + "_" + "_".join(condition_pair) + f'_Condition_TFR_{classifier}.npy'           
        auc     = np.load(auc_in)
        auc = np.mean(auc, axis=0)
        
        # init or append
        if isub==0 and ic==0: 
            aucs = np.zeros((n_subjects, len(config.condition_pairs), auc.data.shape[0]))
            aucs[isub,ic] = auc
        else: 
            aucs[isub, ic] = auc
    ...
    figure, ax          = decoding.plot_decoding_scores(aucs_gdav, epochs, plot_title=f'Betas Av (N={n_subjects}) {"_".join(condition_pair)} Condition', plotting_theme="ticks")  

    report.add_figure(figure, title= f'Betas Av (N={n_subjects}) {"_".join(condition_pair)} Condition')   
    ...
    cluster_stats = permutation_cluster_1samp_test(aucs_condition - 0.65, n_permutations=n_perm, n_jobs=-1, seed=42, tail=1)        
