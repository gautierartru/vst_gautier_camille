...
for ic, condition_pair in enumerate(config.condition_pairs):
    report_out  = report_dir + f'popreg{"_".join(condition_pair)}.html'
    ...
    for isub, subject in enumerate(subjects_list):
        ...
        for ip,p in enumerate(pred_vars):
            ...
            beta_in  = stat_dir + subject + "_"+ pick_type + "_" + epochs_type + "_" + "_".join(condition_pair) + f'_{beta_power_computation}_{p}_reg_' + foi + 'ipower_betas.fif'           
            beta     = mne.read_evokeds(beta_in)[0].pick('eeg')
            
            # init or append
            if isub==0 and ip==0 and ic==0: 
                betas = np.zeros((n_subjects, len(pred_vars), len(config.condition_pairs), beta.data.shape[0], beta.data.shape[1]))
    ...
    for ip,p in enumerate(pred_vars):
        ...
        report.add_figure(figure, title= f'Betas Av (N={n_subjects}) {"_".join(condition_pair)} {p}')   
        plt.close()
