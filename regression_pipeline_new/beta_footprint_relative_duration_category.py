...
roc_auc = roc_auc_score(y_onehot, y_pred_prob, multi_class='ovr')

# Step 3: Plot the original data points and decision boundary
plt.scatter(X[y == 0], y[y == 0], color='black', label='Correct')
plt.scatter(X[y == 1], y[y == 1], color='red', label='Longer')
plt.scatter(X[y == -1], y[y == -1], color='blue', label='Shorter')
...
plt.savefig(report_dir + f'beta_footprint_relative_duration_category_logistic_regression_{t_min}_{t_max}_frontocentral_plot.png')
